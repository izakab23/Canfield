    ...
    : deck(), reserve() // calling default constructors
{
    ...
    fromId = 0; 
    toId   = 0;
    score  = 0;  //counts cards in reserve, 52 means user wins.
} // Canfield() 
...
void Canfield::move()
{
    ...
    if (card != -1 && checkMove(card)) // is the move possible?
    {
        removeCard();
        addCard(card);
        if (score == 52) // if all the cards are in the foundation
            userWin();
    }
    else 
    {
...
void Canfield::addCard(int card)
{
    ...
    // fills the empty tableau as long as reserve still has cards in it
...
// moves the reserve card into the now empty tableau
void Canfield::reserveFill()
{
    int reserveCard;
    reserveCard = reserve.top(); // gets the top card in the reserve
    reserve.pop(); // moves the next card to the top
    tableau[fromId].push(reserveCard); // moves the reserveCard into the empty tableau
} // canfield::reserveFill()

void Canfield::userWin()
{
    char endChoice;
    printGame();
